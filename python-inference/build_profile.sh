...
# Run the container with GPUs and port mapping
...
docker run --rm \
    ...
    ml-workload \
    --output-dir /app/profiling_results \
    --num-inferences 50 \
    2>&1 | tee "$OUTPUT_DIR/container_output.log"
...
curl -s http://localhost:8000/metrics > "$OUTPUT_DIR/prometheus_metrics.txt"
if [ $? -eq 0 ]; then
    log "Successfully captured Prometheus metrics"
else
    log "Warning: Could not access Prometheus metrics endpoint"
fi
...
log "Processing metrics..."
{
    ...
    echo -e "\nPrometheus Metrics:"
    if [ -s "$OUTPUT_DIR/prometheus_metrics.txt" ]; then
        echo "ML metrics available at: http://localhost:8000/metrics"
        echo "Sample metrics:"
        grep -E "^(ml_inference_time|ml_gpu_utilization|ml_tokens_processed)" "$OUTPUT_DIR/prometheus_metrics.txt"
    else
        echo "No Prometheus metrics collected"
    fi
...
echo "- Prometheus metrics: prometheus_metrics.txt"
